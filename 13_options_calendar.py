...
function renderChart(dateStr, t) {{
    ...
    const [spxExitHH, spxExitMM] = t.exit_time.split(':').map(Number);
    const spxExitMins = spxExitHH * 60 + spxExitMM;
    for (const bar of candleData) {{
        const barDate = new Date(bar.time * 1000);
        const barMins = barDate.getUTCHours() * 60 + barDate.getUTCMinutes();
        if (barMins >= spxExitMins) {{ exitTime = bar.time; break; }}
    }}
...
function renderOptionChart(dateStr, t) {{
    ...
    const [optExitHH, optExitMM] = t.exit_time.split(':').map(Number);
    const optExitMins = optExitHH * 60 + optExitMM;
    for (const bar of candleData) {{
        const barDate = new Date(bar.time * 1000);
        const barMins = barDate.getUTCHours() * 60 + barDate.getUTCMinutes();
        if (barMins >= optExitMins) {{ exitTime = bar.time; break; }}
    }}
