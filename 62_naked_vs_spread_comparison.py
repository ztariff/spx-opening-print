...
trades and outputs:
  1. backtest_results/naked_portfolio.json — edge definitions
  2. backtest_results/naked_portfolio_trades.json — all verified trades
  3. naked_trade_calendar.html — interactive calendar
...
OPENING_EDGES = [
    ...
    {
        'name': 'OP4_Trend_VLow_w',
        'label': 'StrongBody_Trend(no_a20d)|VeryLow|Micro_2_1.5_5|long_call',
        'session': 'opening',
        'struct': 'long_call',
        'exit': {'pt_pts': 2, 'sl_pts': 1.5, 'ts_min': 5},
        'color': '#aa66ff',
        ...
        'direction': 1,
        'bars_key': 'bars',
        'note': 'WIDENED — dropped above_20d requirement',
    },
    {
        'name': 'OP5_Triple_VLow_w',
...
CLOSING_EDGES = [
    {
        'name': 'CL1_C2BullHigh_VLow',
        'label': 'C2_BullHigh|VLow|uM_2_1_3|long_call',
        'session': 'closing',
        'struct': 'long_call',
    ...
    {
        'name': 'CL3_StrongBull_Hi_w',
        'label': 'CFB_StrongBull(br>0.65)|Hi_TuWe|Av_5_2_10|long_call',
        'session': 'closing',
        'struct': 'long_call',
    ...
    {
        'name': 'CL6_Reversal_LowTuWe',
        'label': 'CFB_ReversalLow|Low_TuWe|HTC_pure|long_call',
        'session': 'closing',
        'struct': 'long_call',
        'exit': {'pt_pts': None, 'sl_pts': None, 'ts_min': 120},
        'color': '#aaff33',
...
def main():
    ...
    # SAVE JSON
    # ═══════════════════════════════════════════════════════════════
    with open(OUT_DIR / 'naked_portfolio.json', 'w') as f:
        json.dump(portfolio, f, indent=2)
    with open(OUT_DIR / 'naked_portfolio_trades.json', 'w') as f:
    ...
    print(f"\n  Saved: backtest_results/naked_portfolio.json")
    print(f"  Saved: backtest_results/naked_portfolio_trades.json")
    print(f"  Saved: naked_trade_calendar.html")
    print(f"\nDONE")
...
init();
    ...
    with open(SCRIPT_DIR / 'naked_trade_calendar.html', 'w') as f:
        f.write(html)
